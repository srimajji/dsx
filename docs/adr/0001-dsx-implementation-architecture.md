...
```
The common DSX development layer defines a managed-standard-image contract:

- Install Zsh and use `/bin/zsh -il` only for a bare interactive `dsx shell`. Image-level `ZDOTDIR=/usr/local/share/dsx/shell` selects a root-owned `.zshrc` loader, preventing the first-run Zsh wizard even when `/home/dsx` is an empty persistent volume. `/usr/local/share/dsx/shell/dsx.zsh` is the sole authored executable defaults file. The other authored inputs are `/usr/local/share/dsx/shell/zsh_plugins.txt` and `/usr/local/share/dsx/shell/starship.toml`.
- Fetch Antidote and all plugin sources during the image build from immutable checksum- or commit-pinned inputs. The selected plugin order is invariant: `zsh-users/zsh-completions` adds its `fpath` before `compinit`; `Aloxaf/fzf-tab` loads after `compinit`; then `zsh-users/zsh-history-substring-search` and `zsh-users/zsh-autosuggestions` load; `zsh-users/zsh-syntax-highlighting` loads last. Oh My Zsh, nvm plugins, and pyenv plugins are not part of the managed stack.
- Generate `/usr/local/share/dsx/shell/plugins-pre.zsh`, `/usr/local/share/dsx/shell/plugins-post.zsh`, the immutable completion dump `/usr/local/share/dsx/shell/zcompdump`, `/usr/local/share/dsx/shell/fzf-init.zsh`, `/usr/local/share/dsx/shell/direnv-init.zsh`, and `/usr/local/share/dsx/shell/starship-init.zsh` in the image build. `dsx.zsh` starts them in deterministic order: source the pre-loader; run `compinit -C` against the completion dump; source the static FZF initializer when stdin and stdout are terminals; source the post-loader; source the static direnv initializer; then set `STARSHIP_CONFIG` to `/usr/local/share/dsx/shell/starship.toml` and source the static Starship initializer. Pinned Antidote remains installed in `/opt/antidote` and autoloadable from managed Zsh, but startup does not invoke it. Shell startup performs no network access, plugin resolution, checkout, or regeneration.
- Install Node.js active LTS with npm, a compatible stable pnpm, Python 3 with pip and venv plus a `python` command, Go 1.26.5, and a supported LTS JDK with `java` and `javac`. Tool artifacts and installers are immutable and checksum-pinned.
- Publish the toolchain PATH through the image environment, not only through Zsh startup files. A direct `dsx shell -- <command>` remains the exact structured argv passed to the runtime exec path, and `dsx-guest` continues to start configured argv processes without a shell. Neither path depends on rc files; only an explicitly configured shell command invokes one.
- Never read, copy, mount, import, or execute host dotfiles. The complete shell experience is image-owned and does not weaken the existing host-home isolation boundary.
- Apply this contract only when the approved plan selects the DSX-managed standard image. Custom images remain the project's explicit responsibility: DSX does not inject this shell or toolchain layer or change their shell expectations, while the read-only runtime injection of `dsx-guest` remains authoritative.

The shell assets, locks, generated initialization, and toolchain inputs are part of the standard image build context and its content digest. They therefore preserve the existing image-layer cache key, approval review, managed-image attestation, and release-digest semantics rather than creating a second startup-time trust path.
