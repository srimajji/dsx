...
func (service *InspectionService) Inspect(ctx context.Context, request InspectRequest) (InspectResult, error) {
	...
	configAbsolute := filepath.Join(facts.WorkspaceRoot, filepath.FromSlash(projectConfigPath))
	info, statErr := os.Lstat(configAbsolute)
	switch {
	case errors.Is(statErr, os.ErrNotExist):
		result.Diagnostics = append(result.Diagnostics, config.Diagnostic{
			...
			Message:  "no .dsx/config.jsonc exists; detected declarations are shown, but no executable plan was invented",
		})
		...
		return result, nil
	case statErr != nil:
		return result, model.Wrap(model.CodeInvalidInput, "inspect project configuration", statErr)
	case info.Mode()&os.ModeSymlink != 0:
		result.Diagnostics = append(result.Diagnostics, config.Diagnostic{Severity: "error", Code: "config_symlink", Path: projectConfigPath, Message: "project configuration must be a regular file within the canonical project root"})
		sortDiagnostics(result.Diagnostics)
		return result, invalidDiagnosticsError(result.Diagnostics)
	case !info.Mode().IsRegular():
		result.Diagnostics = append(result.Diagnostics, config.Diagnostic{Severity: "error", Code: "config_type", Path: projectConfigPath, Message: "project configuration must be a regular file"})
		sortDiagnostics(result.Diagnostics)
		return result, invalidDiagnosticsError(result.Diagnostics)
	}

	result.Facts.ConfigExists = true
	result.Facts.ConfigPath = projectConfigPath
	validated, parseDiagnostics := service.parseConfig(configAbsolute, projectConfigPath)
